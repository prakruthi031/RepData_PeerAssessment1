...
The total number of NA values in the dataset is **`r no_na`**.
...
```{r total_na_values_new_data}
no_na <- sum(is.na(newdata$steps))
```

The no. of missing NA values in the new dataset is **`r no_na`**.
...
Mean of the total number of steps per day: **`r format(totalsteps_newmean,digits = 8)`**

Median of the total number of steps per day: **`r format(totalsteps_newmedian,digits = 8)`**
